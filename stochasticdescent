...
#update de learning rate, afhankelijk van welk epoch we hebben(exponentiele decay)
def updateLearningRate(t):
    return LEARNINGRATE*math.exp(-Decay*t)
...
def gradientDescent(trainingsdata,network,learningrate,batchsize):
    npy.random.shuffle(trainingsdata)
    datapoints = trainingsdata[:batchsize]
...
def Learn(network,trainingsdata,batchsize):
    learningrate = LEARNINGRATE
    for k in range(AMOUNTEPOCHS):
        #print("learn routine "+str(k))
        learningrate = updateLearningRate(k)
        network = gradientDescent(trainingsdata,network,learningrate,batchsize)
    print("OriginalCost: ",end="")
    print(cost(network,trainingsdata))
...
def main():
    ...
    Learn(network,trainingsdata,batchsize)
